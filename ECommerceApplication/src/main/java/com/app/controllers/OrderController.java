...
public class OrderController {
	...
	@PostMapping("/public/users/{email}/carts/{cartId}/payments/{paymentMethod}/order/{shipmentType}")
	public ResponseEntity<OrderDTO> orderProducts(@PathVariable String email,
												  @PathVariable Long cartId,
												  @PathVariable String paymentMethod,
												  @PathVariable String shipmentType,
												  @RequestParam(name = "couponId", defaultValue = "-1", required = false) Long couponId) {
		OrderDTO order = orderService.placeOrder(email, cartId, paymentMethod, couponId, shipmentType);
